...
int main(int argc, char *argv[]) {
    ...
    Point *points = NULL;
    int num_points = parse_points(&points);

    if (num_points < 0) {
        fprintf(stderr, "Failed to parse points.\n");
        free(points);
        return EXIT_FAILURE;
    }
    // Tests parse_points
    ...
    int all_x_same = 1; // if they differ it will be set to 0
    int all_y_same = 1;
    float mean = calculate_mean(points, num_points, &all_x_same, &all_y_same);

    // Test calculate_mean
    printf("mean = %f\n", mean);

    if(check_base_cases(points, num_points) == 0) {
        free(points);
        return EXIT_SUCCESS;
    }

    Point *left_part = NULL;
    Point *right_part = NULL;
    int left_size = 0;
    int right_size = 0;
    divide_points(points, num_points, mean, all_x_same, &left_part, &right_part, &left_size, &right_size);
    ...
    for (int i = 0; i < left_size; i++) {
        printf("Point %d: (%f, %f)\n", i, left_part[i].x, left_part[i].y);
    }
    printf("Right part:\n");
    for (int i = 0; i < right_size; i++) {
        printf("Point %d: (%f, %f)\n", i, right_part[i].x, right_part[i].y);
    }

    printf("\nhandle_children Test:\n");
    handle_children(left_part, right_part, left_size, right_size);
