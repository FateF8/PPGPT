...
public class Simulation {
    ...
    private List<RegenerativeFoodSource> rFoodSources;
    ...
    private FoodSource spawnFoodSource() {
        ...
        if (Math.random() < 0.2) {
            return new RegenerativeFoodSource(foodX, foodY, (int) (Math.random() * 30) + 20, 100, 1.01);
        } else {
    ...
    public void applyRegeneration() {
        specialPoints.stream()
                .filter(p -> p instanceof RegenerativeFoodSource)
                .map(p -> (RegenerativeFoodSource) p)
                .forEach(RegenerativeFoodSource::regenerate);
    }
    ...
    public void simulateAndDraw() {
        ...
        specialPoints.stream()
                ...
                .forEach(rFS -> {
                    ...
                    cd.fillCircle(rFS.getX(), rFS.getY(), rFS.getFoodAmount() * 0.2);
                });
